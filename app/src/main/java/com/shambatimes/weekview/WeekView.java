...
import static com.shambatimes.weekview.WeekViewUtil.*;
...
public class WeekView extends View {
    ...
    public void setXOriginOffset(float offset) {
        mCurrentOrigin.x = offset;
        goToNearestOrigin();
        invalidate();
    }

    public float getWidthPerStage() {

        float amWidthPerStage = getWidth() - mHeaderColumnWidth - mColumnGap * (getVisibleStageCount());
        amWidthPerStage = amWidthPerStage / getRealNumberOfVisibleStages();

        return amWidthPerStage;
    ...
    private int mMinHourHeight = 120; //no minimum specified (will be dynamic, based on screen)
    ...
    private boolean mShowHalfHours = true;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            //Prevent from scrolling beyond the left/right boundary.
            if (mCurrentOrigin.x - distanceX > 0) {
                distanceX = 0;
            }
            //TODO the 3 needs to be how many items can be off screen
            float paintWidth = ((mWidthPerStage * 3) * -1);
            if (mCurrentOrigin.x - distanceX < paintWidth) {
                distanceX = 0;
            }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (allEvents != null && mEventClickListener != null) {
                List<EventRect> reversedEventRects = allEvents;
                Collections.reverse(reversedEventRects);
                for (EventRect eventRect : reversedEventRects) {
                    if (eventRect.event.getId() != mNewEventId && eventRect.rectF != null)
                        if (e.getX() > eventRect.rectF.left) {
                            if (e.getX() < eventRect.rectF.right) {
                                if (e.getY() > eventRect.rectF.top) {
                                    if (e.getY() < eventRect.rectF.bottom) {
...
//                        float left = 0;
//                        float right = left + mWidthPerStage;
...
//            }
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mEventLongPressListener != null && allEvents != null) {
                List<EventRect> reversedEventRects = allEvents;
                Collections.reverse(reversedEventRects);
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mShowHalfHours = true; //a.getBoolean(R.styleable.WeekView_showHalfHours, mShowHalfHours);
        } finally {
            a.recycle();
        }
    ...
    private void resetHomeDate() {
        ...
        if (mMaxDate != null) {
            ...
            date.add(Calendar.DATE, 1 - getRealNumberOfVisibleStages());
    ...
    private float getXOriginForDate(Calendar date) {
        return -daysBetween(mHomeDate, date) * (mWidthPerStage + mColumnGap);
    }
    ...
    private void calculateHeaderHeight() {
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int dayNumber = 0;
                 dayNumber < getRealNumberOfVisibleStages();
                 dayNumber++) {
    ...
    public void setVisibleStageCount(int stagesVisible) {
        this.stagesVisible = stagesVisible;
    }

    public int getVisibleStageCount() {
        return stagesVisible;
    }

    private ArrayList<EventRect> allEvents = new ArrayList<EventRect>();
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        mWidthPerStage = getWidth() - mHeaderColumnWidth - mColumnGap * (getVisibleStageCount());
        mWidthPerStage = mWidthPerStage / getRealNumberOfVisibleStages();
        ...
        if (mIsFirstDraw) {
            ...
            if (getRealNumberOfVisibleStages() >= 7 && mHomeDate.get(Calendar.DAY_OF_WEEK) != mFirstDayOfWeek && mShowFirstDayOfWeekFirst) {
                int difference = (mHomeDate.get(Calendar.DAY_OF_WEEK) - mFirstDayOfWeek);
                mCurrentOrigin.x += (mWidthPerStage + mColumnGap) * difference;
            }
        }
        ...
        int leftStagesWithGaps = (int) -(Math.ceil(mCurrentOrigin.x / (mWidthPerStage + mColumnGap)));
        float startFromPixel = mCurrentOrigin.x + (mWidthPerStage + mColumnGap) * leftStagesWithGaps +
                mHeaderColumnWidth;
        ...
        lineCount = (lineCount) * (getRealNumberOfVisibleStages() + 1);
        ...
        mFirstVisibleDay.add(Calendar.DATE, -(Math.round(mCurrentOrigin.x / (mWidthPerStage + mColumnGap))));
...
//            mLastVisibleDay.add(Calendar.DATE, stageNumber - 2);
...
//            }
            ...
            //TODO Re consider how this works.
//            if (mEventRects == null || mRefreshEvents
//                    || (stageNumber == leftStagesWithGaps + 1 && mFetchedPeriod != (int) mWeekViewLoader.toWeekViewPeriodIndex(day)
//                    && Math.abs(mFetchedPeriod - mWeekViewLoader.toWeekViewPeriodIndex(day)) > 0.5)) {
//                getMoreEvents(day);
//                mRefreshEvents = false;
//            }
            getMoreEventsByStage(stageNumber);
            ...
            for (int hourNumber = 0; hourNumber < 24; hourNumber++) {
                ...
                if (top > mHeaderHeight + mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom - mHourSeparatorHeight && top < getHeight() && startPixel + mWidthPerStage - start > 0) {
                    hourLines[i * 4] = start;
                    hourLines[i * 4 + 1] = top;
                    hourLines[i * 4 + 2] = startPixel + mWidthPerStage;
                    hourLines[i * 4 + 3] = top;
                    i++;
                }
            }
            ...
            // Draw the events.)
            drawEvents(stageNumber, startPixel, canvas);

            // Draw the line at the current time.
            //if (mShowNowLine && isToday) {
            if (mShowNowLine && stageNumber == 0) {
                ...
                canvas.drawLine(start, startY + beforeNow, startPixel + mWidthPerStage, startY + beforeNow, mNowLinePaint);
            }
...
//        // Draw the header row texts.
        ...
        //  Log.i("START", "Start Pixel: " + startPixel);

        for (int stageNumber = leftStagesWithGaps; stageNumber <= leftStagesWithGaps + getVisibleStageCount(); stageNumber++) {
...
//                continue;
    ...
    private Calendar getTimeFromPoint(float x, float y) {
        int leftDaysWithGaps = (int) -(Math.ceil(mCurrentOrigin.x / (mWidthPerStage + mColumnGap)));
        float startPixel = mCurrentOrigin.x + (mWidthPerStage + mColumnGap) * leftDaysWithGaps +
                mHeaderColumnWidth;
        for (int dayNumber = leftDaysWithGaps + 1;
             dayNumber <= leftDaysWithGaps + getRealNumberOfVisibleStages() + 1;
             dayNumber++) {
            float start = (startPixel < mHeaderColumnWidth ? mHeaderColumnWidth : startPixel);
            if (mWidthPerStage + startPixel - start > 0 && x > start && x < startPixel + mWidthPerStage) {
     ...
     */
    private void drawEvents(int stage, float startFromPixel, Canvas canvas) {
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
...
//            }
     ...
     */
    public int getRealNumberOfVisibleStages() {
        if (mMinDate == null || mMaxDate == null)
            return getNumberOfVisibleStages();
...
//c         Check after call of mGestureDetector, so mCurrentFlingDirection and mCurrentScrollDirection are set.
    ...
    private void goToNearestOrigin() {
        double leftDays = mCurrentOrigin.x / (mWidthPerStage + mColumnGap);
        ...
        int nearestOrigin = (int) (mCurrentOrigin.x - leftDays * (mWidthPerStage + mColumnGap));
        ...
        if (nearestOrigin != 0 && mayScrollHorizontal) {
            ...
            mScroller.forceFinished(true);
            // Snap to date.
            mScroller.startScroll((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, -nearestOrigin, 0, (int) (Math.abs(nearestOrigin) / mWidthPerStage * mScrollDuration));
            ViewCompat.postInvalidateOnAnimation(WeekView.this);
        }
    ...
    public void goToDate(Calendar date) {
        ...
        mCurrentOrigin.x = -daysBetween(mHomeDate, date) * (mWidthPerStage + mColumnGap);
        invalidate();
    }
